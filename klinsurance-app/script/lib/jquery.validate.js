...
jQuery.fn.extend({
	baseValidate: function (param) {
		...
		if (param && param.submitHandler) {
			submitHandler = param.submitHandler;
		}
		return this.validate({
			...
			showErrors: function (errorMap, errorList) {
				...
				if (!!errorList && errorList.length > 0) {
					var errormsg = $(errorList[0].element).attr("errormsg");
...
//固定电话号码验证（支持分机号）
jQuery.validator.addMethod("t", function(value, element) {
